...
TEST(TomateTimer, IdleToActive) {
	...
	LONGS_EQUAL(TomateTimer::ACTIVITY_LENGTH_MS, timer->getTimeOut());
}
...
TEST(TomateTimer, ActiveToBreak) {
	...
	CHECK_FALSE(timer->isOff());
	LONGS_EQUAL(TomateTimer::NORMAL_BREAK_LENGTH_MS, timer->getTimeOut());
}
...
TEST(TomateTimer, longBreakAfterFourActives) {
	...
	CHECK(tomateTimer->currentState() == TomateTimer::BREAK_STATE);
	CHECK_FALSE(timer->isOff());
	LONGS_EQUAL(TomateTimer::LONG_BREAK_LENGTH_MS, timer->getTimeOut());
}
